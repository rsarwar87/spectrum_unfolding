...
public:
  ...
  // non-virtual methods:
  QVector<DataType> * getData() { return &mData; };
  ...
  const_iterator at(int index) { return constBegin()+qBound(0, index, size()); }
  iterator at_mod(int index) { return begin() + qBound(0, index, size()); }
...
public:
  ...
  */
  ...
  void setOpenGl(bool enabled, QOpenGLContext *c, int multisampling = 16);
  ...
  QOpenGLContext* getOGLcontext() { return mGlContext.data()->shareContext(); };
  ...
  Q_SLOT void replot(bool step, QCustomPlot::RefreshPriority refreshPriority = QCustomPlot::rpRefreshHint);
...
protected:
  ...
  bool setupOpenGl(QOpenGLContext *c);
  void freeOpenGl();
